// embind glue for Spark physics engine.
// Spark is a 3D rigid body physics engine compiled to WebAssembly.
//
// Conventions:
// - Vectors are plain JS objects {x, y, z}; quaternions are {x, y, z, w}.
// - Creation functions take a single options object applied over the default
//   def, so every field stays optional.
...
struct Body
{
	...
	{
		b3ShapeDef def = shapeDefFromOpts( opts );
		float height = getFloat( opts, "height", 1.0f );
		float radius = getFloat( opts, "radius", 0.5f );
		int sides = getInt( opts, "sides", 16 );

		// Box3D creates the cylinder as a tessellated hull
		b3HullData* hull = b3CreateCylinder( height, radius, 0.0f, sides );
		if ( hull == nullptr )
		{
			Shape shape = { b3_nullShapeId };
			return shape;
	...
	Shape createMesh( val opts )
	{
		...
		// Build a b3MeshData from the options
		val vertices = opts["vertices"];
		val optsIndices = opts["indices"];
		int vertexCount = vertices["length"].as<int>();
		int triangleCount = optsIndices["length"].as<int>() / 3;
		...
		meshDef.vertices = verts.data();

		// Indices flat array: [i1, i2, i3, i1, i2, i3, ...]
		std::vector<int32_t> indices;
		indices.reserve( triangleCount * 3 );
		for ( int i = 0; i < triangleCount * 3; ++i )
		{
			indices.push_back( optsIndices[i].as<int32_t>() );
		}
		meshDef.indices = indices.data();

		// Material indices (optional)
		std::vector<uint8_t> materials;
		if ( hasKey( opts, "materialIndices" ) )
		{
			val matArr = opts["materialIndices"];
			int matCount = matArr["length"].as<int>();
			materials.reserve( matCount );
			for ( int i = 0; i < matCount; ++i )
			{
				materials.push_back( (uint8_t)matArr[i].as<int>() );
			}
			meshDef.materialIndices = materials.data();
		}
		else
		{
		...
		// Scale (optional)
		b3Vec3 scale = getVec3( opts, "scale", b3Vec3_zero );
		...
		Shape shape = { b3CreateMeshShape( id, &def, meshData, scale ) };
...
struct World
{
	...
	// Callback context for castRayAll
	struct RayAllContext
	{
		std::vector<val> hits;
	};

	// b3CastResultFcn prototype:
	//   float callback(b3ShapeId shapeId, b3Pos point, b3Vec3 normal, float fraction,
	//                  uint64_t userMaterialId, int triangleIndex, int childIndex, void* context)
	// Return values: -1 = filter, 0 = terminate, fraction = clip, 1 = continue
	static float rayAllCallback( b3ShapeId shapeId, b3Pos point, b3Vec3 normal, float fraction,
								 uint64_t userMaterialId, int triangleIndex, int childIndex, void* context )
	{
		(void)userMaterialId;
		(void)triangleIndex;
		(void)childIndex;
		RayAllContext* ctx = static_cast<RayAllContext*>( context );
		val hit = val::object();
		hit.set( "point", fromVec3( point ) );
		hit.set( "normal", fromVec3( normal ) );
		hit.set( "fraction", fraction );
		hit.set( "shapeUserData", tagOf( b3Shape_GetUserData( shapeId ) ) );
		hit.set( "bodyUserData", tagOf( b3Body_GetUserData( b3Shape_GetBody( shapeId ) ) ) );
		Shape shape = { shapeId };
		hit.set( "shape", val( shape ) );
		ctx->hits.push_back( hit );
		// Return 1 to continue traversal — collect ALL hits
		return 1.0f;
	}

	val castRayAll( val origin, val translation, val filterOpts ) const
	{
		RayAllContext ctx;

		b3World_CastRay( id, toVec3( origin, b3Vec3_zero ), toVec3( translation, b3Vec3_zero ),
						 queryFilterFromOpts( filterOpts ), rayAllCallback, &ctx );

		val arr = val::array();
		for ( size_t i = 0; i < ctx.hits.size(); ++i )
		{
			arr.set( (int)i, ctx.hits[i] );
		}
		return arr;
	}
...
EMSCRIPTEN_BINDINGS( spark )
{
	...
	class_<World>( "World" )
		...
		.function( "castRayAll", &World::castRayAll )
